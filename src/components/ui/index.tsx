...
interface BadgeProps { 
  label?: string
  variant?: 'default' | 'success' | 'secondary' | 'danger'
  className?: string
  children?: React.ReactNode
}
export function Badge({ label, variant = 'default', className, children }: BadgeProps) {
  const variants = {
    default: 'bg-slate-200/50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 border-slate-300/50 dark:border-slate-700/50',
    success: 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-500/30 dark:border-emerald-500/30',
    secondary: 'bg-slate-400/15 text-slate-700 dark:text-slate-300 border-slate-400/30 dark:border-slate-400/30',
    danger: 'bg-red-500/15 text-red-700 dark:text-red-400 border-red-500/30 dark:border-red-500/30',
  }
  return (
    <span className={cn('inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-wide border', variants[variant], className)}>
      {children || label}
    </span>
  )
}
